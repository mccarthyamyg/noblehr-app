...
const FROM = process.env.SMTP_FROM || 'PolicyVault <noreply@policyvault.app>';
...
export async function sendPasswordReset({ to, resetLink }) {
  const subject = '[PolicyVault] Reset your password';
  const html = `
...
export async function sendOrgApprovalNotification({ superAdminEmail, orgName, adminEmail, adminName, approvalLink }) {
  const subject = `[PolicyVault] New organization awaiting approval: ${orgName}`;
  const html = `
    <h2>New Organization Signup</h2>
    <p><strong>Organization:</strong> ${orgName}</p>
    <p><strong>Admin:</strong> ${adminName || adminEmail} (${adminEmail})</p>
    <p><a href="${approvalLink}" style="display:inline-block;padding:10px 20px;background:#4f46e5;color:white;text-decoration:none;border-radius:6px;">Approve or Deny</a></p>
...
export async function sendAcknowledgmentReminder({ to, policyTitles, dueDate }) {
  ...
  const subject = '[PolicyVault] Policies awaiting your acknowledgment';
  const html = `
    <h2>Policy Acknowledgment Reminder</h2>
    <p>The following policies require your acknowledgment${dueDate ? ` by ${dueDate}` : ''}:</p>
    <ul>${list}</ul>
    <p>Please log in to PolicyVault to review and acknowledge them.</p>
...
export async function sendAcknowledgmentConfirmation({ to, policyTitle }) {
  const subject = '[PolicyVault] You acknowledged a policy';
  const html = `
    ...
    <p style="color:#666;font-size:12px;">PolicyVault HR</p>
  `;
  return sendEmail({ to, subject, html });
}
...
export async function sendVerificationEmail({ to, verificationLink, orgName }) {
  const subject = `Verify your email for ${orgName || 'PolicyVault'}`;
  const html = `
    <h2>Verify your email</h2>
    <p>Click the link below to verify your email address and activate your account.</p>
    <p><a href="${verificationLink}" style="display:inline-block;padding:10px 20px;background:#4f46e5;color:white;text-decoration:none;border-radius:6px;">Verify Email</a></p>
